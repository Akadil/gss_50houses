...
# file:///Users/akadilkalimoldayev/Desktop/Gunn/Объект. Нарынкол 50 домов
name_bases = ['Заключение технадзора 3 эт улкенбаева', 'Заключение технадзора 3 эт сарыбаева',
              'Заключение технадзора 3 эт сарыбаева', 'Заключение технадзора 4 эт улкенбаева',
              'Заключение технадзора 4 эт сарыбаева', 'Заключение технадзора 4 эт сарыбаева']
...
for m in range(0, 6):
    ...
    for item in number_bases[m]:
        ...
        # print("\n")
        left_position = all_paras[11].text.find('аев') + 4
        strin = all_paras[11].text[left_position:left_position + 7]
        ...
        all_paras[11].text = all_paras[11].text[:left_position] + str(item[0]) + all_paras[11].text[right_position:]

        style = doc.styles['Normal']
        font = style.font
        font.name = 'Times New Roman'
        font.size = Pt(12)
        all_paras[11].style = doc.styles['Normal']

        if m == 0:
            doc.save(f'Нарынкол 23 ({str(item[0])} Улкенбаева) - Заключение технадзор.docx')
        elif m == 1 or m == 2:
            doc.save(f'Нарынкол 23 ({str(item[0])} Сарыбаева) - Заключение технадзор.docx')
        elif m == 3:
            doc.save(f'Нарынкол 27 ({str(item[0])} Улкенбаева) - Заключение технадзор.docx')
        elif m == 4 or m == 5:
            doc.save(f'Нарынкол 27 ({str(item[0])} Сарыбаева) - Заключение технадзор.docx')
        else:
